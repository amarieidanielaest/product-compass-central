...
import { Bell, Mail, MessageSquare, Users, Star, Settings, Clock, Zap } from 'lucide-react';
...
export const NotificationSystem: React.FC<NotificationSystemProps> = ({ boardId }) => {
  ...
  const loadNotificationPreferences = async () => {
    try {
      ...
      // const response = await notificationService.getPreferences(boardId);
  ...
  const savePreferences = async () => {
    try {
      ...
      // Mock API call - replace with actual API
      // await notificationService.updatePreferences(boardId, preferences);
        ...
        <TabsContent value="preferences" className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Receive notifications via email when events occur on your board
              </CardDescription>
            </CardHeader>
                      ...
                      {Object.entries(EVENT_LABELS).map(([event, label]) => (
                        <div key={event} className="flex items-center space-x-2">
                          <input
                            type="checkbox"
                            id={`slack-${event}`}
                            checked={preferences.slack.events.includes(event)}
                            onChange={(e) => {
                              const events = e.target.checked
                                ? [...preferences.slack.events, event]
                                : preferences.slack.events.filter(e => e !== event);
                              updateSlackPreference('events', events);
                            }}
                            className="rounded"
                          />
                          <label htmlFor={`slack-${event}`} className="text-sm">{label}</label>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        <TabsContent value="history" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>View recent notifications sent from this board</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="templates" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Customize notification messages and templates</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="p-4 bg-muted rounded-lg">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Custom notification templates will be available in a future update. 
                    You'll be able to customize the content and format of your notifications.
                  </p>
                </div>
              </div>
            </CardContent>
